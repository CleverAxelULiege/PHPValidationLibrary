...
class MustBeAfterTimeRule extends AbstractRuleTimeOperation{
    ...
    public function isRuleValid(): bool
    {
        ...
        if($this->getIsKey()){
            $this->setMessage("L'heure donnée venant du champs " . $this->getPlaceHolder() . ", " . $value . ", doit être plus tard dans le temps que l'heure que vous avez fournie depuis le champs " . $this->getPlaceHolder($this->getInput()) . ", dont l'heure est " . $valueFromAnotherInput);
        }else{
            $this->setMessage("L'heure donnée venant du champs " . $this->getPlaceHolder() . ", " . $value . ", doit être plus tard dans le temps que " . $valueFromAnotherInput);
        }

        if(ValueHelper::isEmpty($valueFromAnotherInput) == false)
            return DateTimeHelper::isFirstTimeLaterThanSecond($value, $valueFromAnotherInput);
